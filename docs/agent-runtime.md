...
`web_research` 对模型保持一个 Schema。模型根据语义提供 `data_type`、`subject` 和可选的 `time_range_days`，Runtime 再运行对应结构化 adapter 与搜索/抓取管线；只有模型选择 `auto` 或旧模型没有填写类型时，才使用词法识别做兼容兜底。零配置时使用 DuckDuckGo、Bing HTML；生产环境可以在 **设置 → 工具与 MCP → Web Research** 配置 Tavily、SearXNG、Brave Search、Reader 和 GitHub Token，也可继续使用 `TAVILY_API_KEY`（或 `EASYAGENT_TAVILY_API_KEY`）、`EASYAGENT_SEARXNG_URL`、`BRAVE_SEARCH_API_KEY` 等环境变量。页面配置由后端 Provider 注册表驱动，保存值优先于环境变量，并在下一轮 Agent 调用时读取，无需重启。配置型 API provider 优先，候选不足才调用 HTML 降级。`domains` 是抓取前后校验的硬白名单，官方资料请求使用 `source_scope=official`；搜索候选全部失效时，在白名单内通过标准 `sitemap.xml` 恢复站内文档发现，再降级到官网常见入口。候选优先覆盖不同网站域名，短英文实体会先由 Wikidata 返回候选消歧，同时追加精确拼写、meaning 与普通网页查询；版本化文档与正文近重复来源会合并并保留较新版本。中文公司名先由 Wikidata 做实体消歧，再读取行情；GitHub REST API 被限流时自动降级到 GitHub 官方搜索页和仓库页。可选 Reader 为 PDF、动态页面提供正文降级。这些内部凭据不会被写进 Prompt、`web_research` 参数、结果、Trace 或 Bootstrap 响应；但页面保存值仍位于权限受限的本地 SQLite，Shell、Codex 和 stdio MCP 也以同一个服务账号运行，因此它们不是面向不可信租户的隔离边界。
