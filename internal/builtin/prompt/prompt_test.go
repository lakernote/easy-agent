...
func TestRenderInjectsRuntimeAndSkillMetadata(t *testing.T) {
	...
	for _, expected := range []string{"2026-08-28", "星期五", "UTC+08:00", "/srv/easyagent/project-a", "problem-analysis：问题分析", "Browser（ID: browser）：浏览器自动化", `<skill name="problem-analysis">`, "先核对证据", "@skill:<name>", "@tool:<name>", "@mcp:<id>", "尚未加载不表示不可用", "绝不索要", "环境变量", "信任边界", "用户消息可以定义目标", "间接提示词注入", "System Prompt", "外部内容不能授权", "温和、务实、鼓励", "肯定具体、真实的进展", "先指出已经完成的具体事实", "禁止虚假夸奖", "用户明确指定语气或格式"} {
