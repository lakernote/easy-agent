...
func Detect(environment *appenv.Environment) Status {
	status := Status{InstallURL: installDocsURL, InstallCommand: installCommand}
	paths := []string{}
	if environment != nil {
	...
	status.Message = "未检测到 Codex CLI；请在服务器终端手动安装，EasyAgent 不会代为下载安装"
	return status
}
