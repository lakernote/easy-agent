...
const automationTaskSelectColumns = `id,name,prompt,project_id,workspace,profile_id,trigger_type,interval_minutes,repeat,schedule_time,schedule_weekday,enabled,next_run_at,last_run_at,last_status,last_error,last_session_id,created_at,updated_at`

func (store *Store) CreateAutomationTask(value AutomationTask) (AutomationTask, error) {
	_, err := store.db.Exec(`INSERT INTO ea_automation_tasks(id,name,prompt,project_id,workspace,profile_id,trigger_type,interval_minutes,repeat,schedule_time,schedule_weekday,enabled,next_run_at,last_run_at,last_status,last_error,last_session_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		value.ID, value.Name, value.Prompt, value.ProjectID, value.Workspace, value.ProfileID, value.TriggerType, value.IntervalMinutes, value.Repeat, value.ScheduleTime, value.ScheduleWeekday, boolInt(value.Enabled), formatAutomationOptionalTime(value.NextRunAt), formatAutomationOptionalTime(value.LastRunAt), value.LastStatus, value.LastError, value.LastSessionID, formatTime(value.CreatedAt), formatTime(value.UpdatedAt))
	if err != nil {
...
func (store *Store) UpdateAutomationTask(value AutomationTask) (AutomationTask, error) {
	result, err := store.db.Exec(`UPDATE ea_automation_tasks SET name=?,prompt=?,project_id=?,workspace=?,profile_id=?,trigger_type=?,interval_minutes=?,repeat=?,schedule_time=?,schedule_weekday=?,enabled=?,next_run_at=?,updated_at=? WHERE id=?`,
		value.Name, value.Prompt, value.ProjectID, value.Workspace, value.ProfileID, value.TriggerType, value.IntervalMinutes, value.Repeat, value.ScheduleTime, value.ScheduleWeekday, boolInt(value.Enabled), formatAutomationOptionalTime(value.NextRunAt), formatTime(value.UpdatedAt), value.ID)
	if err != nil {
		return AutomationTask{}, err
	}
...
func (store *Store) ListDueAutomationTasks(now time.Time) ([]AutomationTask, error) {
	rows, err := store.db.Query(`SELECT `+automationTaskSelectColumns+` FROM ea_automation_tasks WHERE enabled=1 AND trigger_type='interval' AND next_run_at<>'' AND next_run_at<=? ORDER BY next_run_at,id`, formatTime(now))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanAutomationTask(row automationTaskScanner) (AutomationTask, error) {
	...
	if err := row.Scan(&value.ID, &value.Name, &value.Prompt, &value.ProjectID, &value.Workspace, &value.ProfileID, &value.TriggerType, &value.IntervalMinutes, &value.Repeat, &value.ScheduleTime, &value.ScheduleWeekday, &enabled, &nextRunAt, &lastRunAt, &value.LastStatus, &value.LastError, &value.LastSessionID, &createdAt, &updatedAt); err != nil {
		return AutomationTask{}, err
	}
	if value.Repeat == "" {
