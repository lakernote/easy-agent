...
func (server *Server) getWeixin(response http.ResponseWriter, _ *http.Request) {
	...
	for _, account := range accounts {
		_, connected := server.weixin.pollers[account.ID]
		views = append(views, weixinAccountView{ID: account.ID, Label: account.Label, UserID: maskWeixinID(account.UserID), Enabled: account.Enabled, Connected: connected, CurrentSessionID: account.CurrentSessionID, LastSeenAt: optionalTimeJSON(account.LastSeenAt), LastMessageAt: optionalTimeJSON(account.LastMessageAt), CreatedAt: account.CreatedAt})
	}
