...
export function TracePanel({ session, onLoadOlder, onError, onClose }: { session: Session; onLoadOlder: (id: string, kind: 'messages' | 'events', before: number) => Promise<void>; onError: (value: string) => void; onClose: () => void }) {
  ...
  const tokenSub = session.usage.totalTokens > 0 ? `入 ${session.usage.inputTokens} · 出 ${session.usage.outputTokens}` : 'Provider 未返回 usage'
  ...
  return <aside className="trace-panel"><div className="trace-head"><div><p className="eyebrow">AUDITABLE RUNTIME</p><h2>Agent Trace</h2></div><button aria-label="关闭 Agent Trace" onClick={onClose}>×</button></div><div className="trace-runtime-banner"><strong>{runtimeName}</strong><span>{session.status === 'running' || session.status === 'queued' ? session.runProgress || '正在处理…' : session.status === 'failed' ? '本轮失败' : '本轮已完成'}</span></div><div className="metrics"><Metric label="LLM" value={`${session.usage.modelCalls} 次`} sub={formatDuration(session.usage.modelDurationMs)} /><Metric label="工具" value={`${session.usage.toolCalls} 次`} sub={formatDuration(session.usage.toolDurationMs)} /><Metric label="Token" value={tokenValue} sub={tokenSub} /><Metric label="Prompt Cache" value={session.usage.cacheReported ? `${cacheRate}%` : '未上报'} sub={session.usage.cacheReported ? `命中 ${session.usage.cachedTokens} · 写入 ${session.usage.cacheWriteTokens}` : 'Provider 未返回缓存字段'} /></div><div className="context-ledger"><div><span>最近上下文</span><strong>{context.lastInputTokens ? formatTokens(context.lastInputTokens) : isCodexRuntime ? 'Codex 未上报' : session.status === 'failed' ? '未上报' : '—'}{context.contextWindowTokens ? ` / ${formatTokens(context.contextWindowTokens)}` : ''}</strong></div><div><span>会话历史</span><strong>{isCodexRuntime ? `Codex thread · ${context.userTurns} 个用户轮次` : `${context.userTurns} 个用户轮次 · ${context.historyMessages} 条消息${session.messagesTruncated ? ' · 历史窗口' : ''}`}</strong></div><div><span>发送方式</span><strong>{isCodexRuntime ? 'Codex thread/resume' : historyModeLabel(context.historyMode)}</strong></div><div><span>压缩</span><strong>{isCodexRuntime ? 'Codex 管理' : context.compressionCount > 0 ? `${context.compressionCount} 次 · ${context.compressedMessages} 条` : context.compressionMode === 'auto' ? `自动 ${context.compressionThresholdPercent}%` : '已停用'}</strong></div></div>{session.eventsTruncated && <div className="trace-empty">Trace 较多；点击下方按钮加载更早记录</div>}{session.eventsHasMore && session.events.length > 0 && <button className="history-load-more" onClick={loadOlder} disabled={loadingOlder}>{loadingOlder ? '加载中…' : '加载更早 Trace'}</button>}<div className="trace-events">{session.events.length === 0 && <div className="trace-empty">还没有 Trace</div>}{session.events.map((event) => <TraceRow key={event.id} event={event} />)}</div></aside>
}
