...
import { FolderIcon, Icon, Logo, MoreIcon } from './ui'
...
export function Sidebar({ page, data, session, onPage, onOpen, onNew, onSession, onRefresh, onLoadOlder, onError }: { page: Page; data: Bootstrap; session: Session | null; onPage: (page: Page) => void; onOpen: (id: string) => void; onNew: () => void; onSession: (value: Session) => void; onRefresh: () => Promise<Bootstrap>; onLoadOlder: (beforeUpdatedAt: string, beforeID: string) => Promise<void>; onError: (value: string) => void }) {
  ...
  return <aside className="sidebar">
    ...
    <div ref={sessionListRef} className="session-list">
      ...
      {projectGroups.map((group) => <section className="session-project" key={group.project?.id || 'unassigned'}><div className="session-project-head" title={group.project?.directories.join('\n') || '未归入项目'}><span><FolderIcon /><strong>{group.project?.name || '历史会话'}</strong><small>{group.sessions.length}</small></span>{group.project && <button type="button" aria-label={`编辑项目 ${group.project.name}`} title="编辑项目" onClick={() => openProject(group.project)}><MoreIcon /></button>}</div>{group.sessions.map(renderSession)}</section>)}
    </div>
    ...
    {projectDialogOpen && <ProjectDialog project={editingProject} busy={savingMetadata} onCancel={() => { setProjectDialogOpen(false); setEditingProject(null) }} onSave={(value) => void saveProject(value)} onDelete={() => editingProject && setPendingProjectDelete(editingProject)} />}
