...
export function Sidebar({ page, data, session, onPage, onOpen, onNew, onSession, onRefresh, onLoadOlder, onError }: { page: Page; data: Bootstrap; session: Session | null; onPage: (page: Page) => void; onOpen: (id: string) => void; onNew: () => void; onSession: (value: Session) => void; onRefresh: () => Promise<Bootstrap>; onLoadOlder: (beforeUpdatedAt: string, beforeID: string) => Promise<void>; onError: (value: string) => void }) {
  ...
  return <aside className="sidebar">
    ...
    <button className={`automation-nav ${page === 'automations' ? 'active' : ''}`} type="button" aria-current={page === 'automations' ? 'page' : undefined} onClick={() => onPage('automations')}><Icon name="automation" /><span><strong>自动化任务</strong><small>手动与定时触发</small></span></button>
    <nav className="primary-nav" aria-label="主导航"><button className={page === 'chat' ? 'active' : ''} aria-current={page === 'chat' ? 'page' : undefined} onClick={() => onPage('chat')}><Icon name="chat" />对话</button><button className={page === 'automations' ? 'active' : ''} aria-current={page === 'automations' ? 'page' : undefined} onClick={() => onPage('automations')}><Icon name="automation" />自动化</button></nav>
