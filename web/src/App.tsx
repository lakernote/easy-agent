...
function Sidebar({ page, data, session, onPage, onOpen, onNew, onRefresh }: { page: Page; data: Bootstrap; session: Session | null; onPage: (page: Page) => void; onOpen: (id: string) => void; onNew: () => void; onRefresh: () => Promise<Bootstrap> }) {
  ...
  return <aside className="sidebar">
    <div className="brand"><div className="brand-mark">E</div><div><strong>EasyAgent</strong><small>ONE CORE AGENT</small></div></div>
...
function Chat({ session, data, onSession, onRefresh, onError }: { session: Session | null; data: Bootstrap; onSession: (session: Session) => void; onRefresh: () => Promise<Bootstrap>; onError: (value: string) => void }) {
  ...
  return <section className="chat-page">
    <div className="conversation">
      {!session && <div className="welcome"><div className="agent-orb"><span>EA</span></div><p className="eyebrow">ONE AGENT · ANY TASK</p><h1>想解决什么问题？</h1><p>无需先建项目。直接提问；需要代码、日志或外部系统时，再添加上下文、Skill 或 MCP。</p><div className="suggestions">{suggestions.map((value) => <button key={value} onClick={() => send(value)}>{value}<span>↗</span></button>)}</div></div>}
...
function Skills({ data, onRefresh, onError }: { data: Bootstrap; onRefresh: () => Promise<Bootstrap>; onError: (value: string) => void }) {
  ...
  return <section className="settings-page">
    <div className="page-intro"><p className="eyebrow">PROGRESSIVE DISCLOSURE</p><h1>Skills</h1><p>System Prompt 保持很小；只有任务相关时，Agent 才通过 <code>load_skill</code> 读取完整说明，减少无效 Token。</p></div>
...
function Capabilities({ data, onRefresh, onError }: { data: Bootstrap; onRefresh: () => Promise<Bootstrap>; onError: (value: string) => void }) {
  ...
  return <section className="settings-page capabilities"><div className="page-intro"><p className="eyebrow">PLUGGABLE CAPABILITIES</p><h1>模型与工具</h1><p>模型、内置 Tool、MCP 和基础 Prompt 分开管理；启用后统一注册给同一个核心 Agent。</p></div>
    <div className="section-block"><div className="section-heading"><div><h2>模型</h2><p>支持 OpenAI Chat Completions 和 Responses 兼容接口。</p></div><span className="tag">{model.protocol}</span></div><div className="form-grid"><label>提供方<input value={model.provider} onChange={(e) => setModel({ ...model, provider: e.target.value })} /></label><label>协议<select value={model.protocol} onChange={(e) => setModel({ ...model, protocol: e.target.value as ModelSettings['protocol'] })}><option value="chat_completions">Chat Completions</option><option value="responses">Responses</option></select></label><label className="wide">Base URL<input value={model.baseUrl} onChange={(e) => setModel({ ...model, baseUrl: e.target.value })} /></label><label>模型名称<input value={model.model} onChange={(e) => setModel({ ...model, model: e.target.value })} /></label><label>最大输出 Token<input type="number" value={model.maxOutputTokens} onChange={(e) => setModel({ ...model, maxOutputTokens: Number(e.target.value) })} /></label><label>模型超时（秒）<input type="number" min="30" max="600" value={model.requestTimeoutSeconds || 300} onChange={(e) => setModel({ ...model, requestTimeoutSeconds: Number(e.target.value) })} /><small>本地大模型建议 300 秒；单次 HTTP 请求最多 600 秒</small></label><label>上下文窗口 Token<input type="number" min="0" value={model.contextWindowTokens || 0} onChange={(e) => setModel({ ...model, contextWindowTokens: Number(e.target.value) })} /><small>0 表示未知；Ollama 运行后读取当前实际窗口</small></label><label>自动压缩阈值<input type="number" min="50" max="90" value={model.compressionThresholdPercent || 75} onChange={(e) => setModel({ ...model, compressionThresholdPercent: Number(e.target.value) })} /><small>达到上下文窗口的该比例后生成检查点，建议 75%</small></label><label>API Key<input type="password" placeholder={model.secretConfigured ? '已配置，留空不修改' : '可留空'} value={model.apiKey || ''} onChange={(e) => setModel({ ...model, apiKey: e.target.value })} /></label><label>API Key 环境变量<input placeholder="例如 OPENAI_API_KEY" value={model.apiKeyEnv || ''} onChange={(e) => setModel({ ...model, apiKeyEnv: e.target.value })} /></label></div><div className="form-actions"><button className="primary-button" onClick={saveModel}>保存模型</button></div><div className="ollama-strip"><div><strong><span className={`service-dot ${data.ollama.running ? '' : 'off'}`} />Ollama</strong><small>{data.ollama.message}</small></div><div>{data.ollama.models.map((item) => <button key={item.name} className="ghost-button" onClick={() => useOllama(item.name)}>使用 {item.name}</button>)}</div></div></div>
    <div className="section-block"><div className="section-heading"><div><h2>内置 Tools</h2><p>随 Go 二进制发布，无需安装。</p></div><span className="tag">{data.builtinTools.length} 个</span></div><div className="tool-table">{data.builtinTools.map((tool) => <div key={tool.name}><code>{tool.name}</code><span>{tool.description}</span><em>{tool.source}</em></div>)}</div></div>
    <div className="section-block"><div className="section-heading"><div><h2>MCP Servers</h2><p>启用前先检测依赖和连接；远端工具统一转换成 <code>mcp__服务__工具</code>。</p></div><button className="ghost-button" onClick={() => setMCP({ id: `mcp-${Date.now()}`, name: 'New MCP', description: '', enabled: false, transport: 'http', args: [], headers: {}, environment: {} })}>＋ 自定义</button></div>{mcpNotice && <div className={`mcp-notice ${mcpNotice.ready ? 'ready' : 'failed'}`}><div><strong>{mcpNotice.title}</strong><span>{mcpNotice.message}</span></div>{mcpNotice.tools.length > 0 && <details><summary>查看 {mcpNotice.tools.length} 个工具</summary><code>{mcpNotice.tools.join('\n')}</code></details>}<button onClick={() => setMCPNotice(null)}>×</button></div>}<div className="mcp-grid">{data.mcps.map((item) => { const preset = data.mcpPresets.find((candidate) => candidate.id === item.id); const canInstall = !item.enabled && preset?.action === 'install'; return <div className="mcp-row" key={item.id}><div><span className={`status ${item.enabled ? 'idle' : 'off'}`} /><strong>{item.name}</strong><small>{item.description || (item.transport === 'stdio' ? `${item.command} ${item.args.join(' ')}` : item.endpoint)}</small></div><span>{item.enabled ? '已启用' : '已停用'}</span><button disabled={installingPreset === item.id} onClick={() => canInstall && preset ? installPreset(preset) : testMCP(item.id)}>{installingPreset === item.id ? '检测中…' : canInstall ? '检测并启用' : '测试'}</button><button onClick={() => setMCP({ ...item })}>编辑</button></div> })}</div><div className="presets"><span>推荐能力</span>{data.mcpPresets.filter((preset) => !data.mcps.some((item) => item.id === preset.id)).map((preset) => <button key={preset.id} disabled={!!installingPreset} onClick={() => installPreset(preset)}><strong>{preset.name}</strong><small>{preset.description}</small><em>{preset.requirement}</em><b>{installingPreset === preset.id ? '正在检测…' : preset.action === 'install' ? '检测并启用' : '配置'}</b></button>)}</div></div>
    ...
    {mcp && <div className="modal-backdrop" onMouseDown={() => setMCP(null)}><div className="modal" onMouseDown={(e) => e.stopPropagation()}>
      <div className="modal-head"><div><p className="eyebrow">MCP SERVER</p><h2>{mcp.name}</h2></div><button onClick={() => setMCP(null)}>×</button></div>
