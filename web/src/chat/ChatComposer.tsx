...
export function ChatComposer(model: ChatComposerModel) {
  ...
  return <div className="composer-wrap"><div ref={composerRef} className={`composer ${dragging ? 'dragging' : ''}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true) }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false) }} onDrop={(event) => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files) }}>
      ...
      <textarea ref={textareaRef} value={draft} onChange={updateDraft} aria-label="消息内容" aria-describedby="composer-help attachment-error" placeholder={!session && !readiness.canSend ? '请先完成运行配置…' : attachments.length ? '描述如何处理这些附件…' : `给 ${isCodexRuntime ? 'Codex' : 'EasyAgent'} 发消息…`} rows={1} onPaste={(event) => {
        const files = Array.from(event.clipboardData.files)
        const itemFiles = files.length ? files : Array.from(event.clipboardData.items).filter((item) => item.kind === 'file').map((item) => item.getAsFile()).filter((file): file is File => Boolean(file))
        if (itemFiles.length) { event.preventDefault(); addFiles(itemFiles) }
      }} onKeyDown={(event) => { if (handleCapabilityKey(event)) return; if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); send() } }} />
