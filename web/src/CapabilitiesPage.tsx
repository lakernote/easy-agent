...
export function Capabilities({ data, onRefresh, onError }: { data: Bootstrap; onRefresh: () => Promise<Bootstrap>; onError: (value: string) => void }) {
  ...
  return <section className={`settings-page capabilities ${codex ? 'codex' : 'easyagent'}`}>
    <div className="page-intro"><p className="eyebrow">运行时配置</p><h1>{codex ? 'Codex Runtime' : 'EasyAgent Runtime'}</h1><p>{codex ? 'Codex app-server 负责 Agent 循环、thread、工具、Skill、沙箱、审批和实时事件。' : 'EasyAgent Go 负责 Agent 循环、工具调用、MCP、Skill 和上下文压缩。'}</p></div>
    <div className="section-block runtime-section"><div className="section-heading"><div><h2>运行时</h2><p>新会话会固定创建时选择的 Runtime；切换不会改变已有会话。</p></div><span className="tag">{codex ? 'Codex' : 'EasyAgent'}</span></div>
      <div className="runtime-picker" role="radiogroup" aria-label="选择 Agent Runtime">
        <label className={!codex ? 'selected' : ''}><input type="radio" name="runtime" value="easyagent" checked={!codex} onChange={() => selectRuntime('easyagent')} /><span><strong>EasyAgent Runtime</strong><small>Go Agent + Ollama/OpenAI 兼容模型；Tools、Skills、MCP 由 EasyAgent 管理。</small></span></label>
        <label className={codex ? 'selected' : ''}><input type="radio" name="runtime" value="codex" checked={codex} onChange={() => selectRuntime('codex')} /><span><strong>Codex Runtime</strong><small>Codex app-server + 本机 Codex 账号配置；thread、工具、Skill、沙箱和审批由 Codex 管理。</small></span></label>
      </div>
