...
export function Capabilities({ section, initialSection, data, onRefresh, onError }: { section: CapabilitiesSection; initialSection?: SettingsSection; data: Bootstrap; onRefresh: () => Promise<Bootstrap>; onError: (value: string) => void }) {
  ...
  const activeRuntimeLabel = runtimeChanged ? '待设为默认' : '当前默认'
  ...
  const visibleModelProfiles = modelRuntimeFilter === 'codex' ? codexProfiles : easyAgentProfiles
  const selectedRuntimeName = modelRuntimeFilter === 'codex' ? 'Codex' : 'EasyAgent'
  const selectedRuntimeProfiles = codex ? codexProfiles : easyAgentProfiles
  const selectRuntimeContext = (runtime: ModelSettings['runtime']) => {
    setModelRuntimeFilter(runtime)
    selectRuntime(runtime)
  }
  const openRuntimeModels = () => {
    setModelRuntimeFilter(model.runtime)
  ...
  return <section className={`settings-page capabilities ${codex ? 'codex' : 'easyagent'} ${section === 'tools' ? 'extensions-page' : 'runtime-page'}`}>
    ...
    {(section === 'runtime' || section === 'settings') && <nav className="settings-section-nav" aria-label="设置分区" role="tablist">
      ...
      <button className={settingsSection === 'tasks' ? 'active' : ''} type="button" role="tab" aria-selected={settingsSection === 'tasks'} onClick={() => setSettingsSection('tasks')}><span>02</span><strong>任务设置</strong><small>并发 / 超时 / 恢复</small></button>
      <button className={settingsSection === 'models' ? 'active' : ''} type="button" role="tab" aria-selected={settingsSection === 'models'} onClick={() => setSettingsSection('models')}><span>03</span><strong>模型配置</strong><small>{data.modelProfiles.length} 套可选配置</small></button>
      <button className={settingsSection === 'skills' ? 'active' : ''} type="button" role="tab" aria-selected={settingsSection === 'skills'} onClick={() => setSettingsSection('skills')}><span>04</span><strong>Skills</strong><small>{data.skills.filter((item) => item.enabled).length}/{data.skills.length} 已启用</small></button>
      <button className={settingsSection === 'tools' ? 'active' : ''} type="button" role="tab" aria-selected={settingsSection === 'tools'} onClick={() => setSettingsSection('tools')}><span>05</span><strong>工具与 MCP</strong><small>{data.builtinTools.length + data.mcps.length} 项能力</small></button>
      <button className={settingsSection === 'usage' ? 'active' : ''} type="button" role="tab" aria-selected={settingsSection === 'usage'} onClick={() => setSettingsSection('usage')}><span>06</span><strong>用量</strong><small>按时间和模型</small></button>
    </nav>}
    ...
    <div className={`runtime-workbench settings-view-${settingsSection}`}>
      {(section === 'runtime' || section === 'settings') && settingsSection === 'runtime' && <nav className="runtime-rail" aria-label="选择 Agent Runtime">
        <div className="runtime-rail-head"><div><p className="eyebrow">Runtime</p><strong>选择执行引擎</strong></div><small>查看配置与运行状态</small></div>
        <button className={`runtime-nav-item ${!codex ? 'selected' : ''}`} type="button" onClick={() => selectRuntimeContext('easyagent')} aria-pressed={!codex}>
          <span className="runtime-nav-dot easyagent-dot" /><span><strong>EasyAgent</strong><small>Go Agent · Ollama / OpenAI</small></span><em>{!persistedCodex ? '当前默认' : `${easyAgentProfiles.length} 套配置`}</em>
        </button>
        <button className={`runtime-nav-item ${codex ? 'selected' : ''}`} type="button" onClick={() => selectRuntimeContext('codex')} aria-pressed={codex}>
          <span className={`runtime-nav-dot ${data.codex.installed && data.codex.appServerAvailable ? 'ready' : ''}`} /><span><strong>Codex</strong><small>app-server · thread / sandbox</small></span><em>{persistedCodex ? '当前默认' : `${codexProfiles.length} 套配置`}</em>
        </button>
        <div className="runtime-rail-foot">选择只切换当前查看内容；点击“设为默认”后，新会话才会使用该 Runtime。</div>
      </nav>}
      <div className="runtime-main">
        {settingsSection === 'runtime' && <>
        <div className="runtime-main-head"><div><p className="eyebrow">{runtimeChanged ? '待设为默认' : '当前默认运行时'}</p><h2>{codex ? 'Codex Runtime' : 'EasyAgent Runtime'}</h2><p>{runtimeChanged ? `设为默认后，下一次新会话将使用 ${codex ? 'Codex app-server' : 'EasyAgent Go'}；已有会话不变。` : codex ? 'Codex app-server 负责 Agent 循环、thread、工具、Skill、沙箱、审批和实时事件。' : 'EasyAgent Go 负责 Agent 循环、工具调用、MCP、Skill 和上下文压缩。'}</p></div><div className="runtime-main-head-actions"><span className={`runtime-state ${runtimeChanged ? 'pending' : 'active'}`}>{activeRuntimeLabel}</span>{runtimeChanged && <button className="primary-button runtime-enable-button" disabled={savingModel} onClick={() => void enableRuntime()}>{savingModel ? '设置中…' : `设为默认 Runtime`}</button>}</div></div>
        {codex && <CodexStatus data={data} onDetect={detectCodex} />}
        <div className="runtime-summary"><div><p className="eyebrow">当前选中配置</p><strong>{model.profileName || '尚未保存配置'}</strong><span>{codex ? 'Codex Runtime' : 'EasyAgent Runtime'} · {model.model || (codex ? '跟随 ~/.codex/config.toml' : '未填写模型')}</span></div><div><p className="eyebrow">此 Runtime</p><strong>{selectedRuntimeProfiles.length} 套配置</strong><span>{runtimeChanged ? '尚未设为新会话默认' : '当前用于创建新会话'}</span></div><button className="ghost-button" type="button" onClick={openRuntimeModels}>管理 {codex ? 'Codex' : 'EasyAgent'} 配置 <span aria-hidden="true">→</span></button></div>
        </>}
        {settingsSection === 'tasks' && <RuntimeOperationsSettings data={data} onRefresh={onRefresh} onError={onError} />}
        {settingsSection === 'models' && <>
        <div className="model-catalog">
          <div className="model-catalog-head"><div><p className="eyebrow">Runtime 与模型</p><h2>模型配置</h2><p>先选择 Runtime，再管理它的 Provider 和模型配置。不同执行引擎之间不会混用参数。</p></div><span className="settings-count">{data.modelProfiles.length} 套配置</span></div>
          <div className="model-runtime-tabs" role="tablist" aria-label="按 Runtime 查看模型配置">
            ...
            <button className={modelRuntimeFilter === 'codex' ? 'selected' : ''} type="button" role="tab" aria-selected={modelRuntimeFilter === 'codex'} onClick={() => selectRuntimeContext('codex')}><span className={`runtime-nav-dot ${data.codex.installed && data.codex.appServerAvailable ? 'ready' : ''}`} /><span><strong>Codex</strong><small>app-server 与 config.toml</small></span><em>{codexProfiles.length} 套{data.model.runtime === 'codex' ? ' · 当前默认' : ''}</em></button>
          </div>
          <div className="model-catalog-toolbar"><div><strong>{selectedRuntimeName} 配置</strong><small>{visibleModelProfiles.length ? '选择一套配置编辑，或将它设为新会话默认。' : '当前 Runtime 还没有保存的配置。'}</small></div><button className="primary-button" type="button" onClick={() => openProfileEditor(undefined, modelRuntimeFilter)}>＋ 新建 {selectedRuntimeName} 配置</button></div>
          <div className="model-profile-directory" aria-label={`${selectedRuntimeName} 模型配置列表`}>
            {visibleModelProfiles.map((profile) => { const profileCodex = profile.settings.runtime === 'codex'; const isDefault = profile.id === data.activeModelProfileId; return <div className={`model-profile-row ${isDefault ? 'selected' : ''}`} key={profile.id}><button className="model-profile-select" type="button" onClick={() => openProfileEditor(profile)} aria-label={`编辑 ${profile.name}`}><span className={`runtime-nav-dot ${profileCodex ? (data.codex.installed && data.codex.appServerAvailable ? 'ready' : '') : 'easyagent-dot'}`} /><span><strong>{profile.name}</strong><small>{profileCodex ? 'app-server' : profile.settings.provider || 'Provider 未设置'} · {profile.settings.model || (profileCodex ? '跟随 config.toml' : '模型未设置')}</small></span></button><div className="model-profile-row-actions">{isDefault ? <span className="profile-default-state"><span className="service-dot" />当前默认</span> : <button className="profile-activate" type="button" disabled={savingModel} onClick={() => activateProfile(profile)}>设为默认</button>}<button className="profile-edit" type="button" onClick={() => openProfileEditor(profile)}>编辑</button></div></div>})}
            {!visibleModelProfiles.length && <div className="model-empty-state"><span className={`runtime-nav-dot ${modelRuntimeFilter === 'easyagent' ? 'easyagent-dot' : ''}`} /><div><strong>还没有 {selectedRuntimeName} 配置</strong><small>新建后可以先测试连接，再设为新会话默认。</small></div><button className="ghost-button" type="button" onClick={() => openProfileEditor(undefined, modelRuntimeFilter)}>新建配置</button></div>}
            ...
            <div className="directory-foot">当前默认配置会用于之后创建的新会话；已有会话不会随配置切换。</div>
          </div>
        </div>
        </>}
        ...
        {settingsSection === 'tools' && <>
          {!codex && <div className="section-block"><div className="section-heading"><div><h2>EasyAgent 内置 Tools</h2><p>当前时间、Shell、只读文件检索和网页检索首轮可用；写入与低频能力按需加载。</p></div><span className="tag">{data.builtinTools.length} 个</span></div><div className="capability-note"><strong>工作区</strong><span><code>{data.runtime.workspace}</code></span><strong>私有 Runtime</strong><span><code>{data.runtime.runtime}</code></span></div><div className="tool-table">{data.builtinTools.map((tool) => <div key={tool.name}><code>{tool.name}</code><span>{tool.description}</span><em>{tool.category || tool.source}</em></div>)}</div></div>}
