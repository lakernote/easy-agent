    ...
    <div className={`runtime-workbench settings-view-${settingsSection}`}>
      <div className="runtime-main">
		...
		{settingsSection === 'tools' && <>
		  ...
		  {!codex && <div className="section-block"><div className="section-heading"><div><h2>EasyAgent 内置 Tools</h2><p>可写会话首轮提供 read、shell、edit、write；只读会话提供 read、grep、find、ls；其余能力按需加载。</p></div><span className="tag">{data.builtinTools.length} 个</span></div><div className="capability-note"><strong>工作区</strong><span><code>{data.runtime.workspace}</code></span><strong>私有 Runtime</strong><span><code>{data.runtime.runtime}</code></span></div><div className="tool-table">{data.builtinTools.map((tool) => <div key={tool.name}><code>{tool.name}</code><span>{tool.description}</span><em>{tool.category || tool.source}</em></div>)}</div></div>}
