...
import { api } from './api'
import type { Bootstrap, CodexProviderConfig, MCPConfig, ModelProfile, ModelSettings } from './types'
...
function CodexStatus({ data, installing, onInstall, onDetect }: { data: Bootstrap; installing: boolean; onInstall: () => void; onDetect: () => void }) {
  const ready = data.codex.installed && data.codex.appServerAvailable
  return <div className={`runtime-status ${ready ? 'ready' : 'missing'}`} role="status" aria-live="polite"><div><strong><span className={`service-dot ${ready ? '' : 'off'}`} />{ready ? `Codex CLI + app-server 已就绪${data.codex.version ? ` · ${data.codex.version}` : ''}` : data.codex.installed ? '已找到 CLI，但 app-server 不可用' : '未检测到 Codex CLI'}</strong><small>{data.codex.installed ? `${data.codex.path} · app-server 是 CLI 的子命令，不需要另装一个 app` : data.codex.message}</small></div><div className="runtime-status-actions">{!data.codex.installed && <button className="primary-button" type="button" disabled={installing} onClick={onInstall}>{installing ? '安装中…' : '在服务器安装 Codex CLI'}</button>}{!data.codex.installed && <a className="ghost-button" href={data.codex.installUrl} target="_blank" rel="noreferrer">安装说明</a>}<button className="ghost-button" onClick={onDetect}>重新检测</button></div>{!data.codex.installed && <code className="runtime-install-command">{data.codex.installCommand}</code>}</div>
}
...
function CodexSettings({ data, config, setConfig, model, setModel, notice, testing, saving, savingConfig, onTest, onSave, onSaveConfig }: { data: Bootstrap; config: CodexProviderConfig; setConfig: (value: CodexProviderConfig) => void; model: ModelSettings; setModel: (value: ModelSettings) => void; notice: Notice | null; testing: boolean; saving: boolean; savingConfig: boolean; onTest: () => void; onSave: () => void; onSaveConfig: (value: CodexProviderConfig & { apiKey?: string; clearApiKey?: boolean }) => void }) {
  const [apiKey, setAPIKey] = useState('')
  const [clearAPIKey, setClearAPIKey] = useState(false)
  const clearApiKey = clearAPIKey
  const update = (value: Partial<CodexProviderConfig>) => setConfig({ ...config, ...value })
  return <div className="section-block codex-config"><div className="section-heading"><div><h2>Codex 连接</h2><p>这里管理服务器上的 <code>~/.codex/config.toml</code>；API Key 只写入服务器私有密钥文件，不会回显。</p></div><span className="tag">app-server</span></div><div className="runtime-boundary-note"><strong>服务器部署</strong><span>Codex CLI 和 app-server 是同一个安装包；EasyAgent 会用当前服务用户启动 app-server，并自动带上这里保存的环境变量。</span></div><div className="codex-config-summary"><span className={`service-dot ${config.configured ? '' : 'off'}`} /><strong>{config.configured ? `${config.providerName || config.provider} 已配置` : '还没有完整配置'}</strong><small>{config.configPath || '~/.codex/config.toml'}{config.apiKeyConfigured ? ' · API Key 已配置' : ' · API Key 未配置'}</small></div>{config.warning && <div className="codex-config-warning" role="alert">{config.warning}</div>}<div className="form-grid"><label>Provider ID<input value={config.provider} onChange={(event) => update({ provider: event.target.value })} placeholder="groq" /><small>例如 groq；不要填写 API Key。</small></label><label>显示名称<input value={config.providerName} onChange={(event) => update({ providerName: event.target.value })} placeholder="Groq" /></label><label className="wide">Base URL<input value={config.baseUrl} onChange={(event) => update({ baseUrl: event.target.value })} placeholder="https://api.groq.com/openai/v1" /></label><label>默认模型<input value={config.model} onChange={(event) => update({ model: event.target.value })} placeholder="openai/gpt-oss-20b" /></label><label>推理强度<select value={config.reasoningEffort} onChange={(event) => update({ reasoningEffort: event.target.value })}><option value="">Provider 默认</option><option value="low">low</option><option value="medium">medium</option><option value="high">high</option><option value="xhigh">xhigh</option></select></label><label>API Key 环境变量<input value={config.envKey} onChange={(event) => update({ envKey: event.target.value })} placeholder="GROQ_API_KEY" /><small>这里填变量名，不是 gsk_... 密钥。</small></label><label className="wide">API Key<input type="password" autoComplete="new-password" value={apiKey} onChange={(event) => { setAPIKey(event.target.value); setClearAPIKey(false) }} placeholder={config.apiKeyConfigured ? '已配置，留空保持不变' : '粘贴 Groq API Key'} /><small>只发送到当前 EasyAgent 服务器；不会写入 config.toml。</small></label>{config.apiKeyConfigured && <label className="check-label wide"><input type="checkbox" checked={clearAPIKey} onChange={(event) => { setClearAPIKey(event.target.checked); if (event.target.checked) setAPIKey('') }} />清除已保存的 API Key</label>}<label className="wide">本配置的模型 override（可选）<input value={model.model} onChange={(event) => setModel({ ...model, model: event.target.value })} placeholder="留空：使用上面的默认模型" /><small>多个 Codex 配置可以用不同 override；留空时使用 config.toml 默认模型。</small></label><label>请求超时（秒）<input type="number" min={data.modelRules.minRequestTimeoutSeconds} max={data.modelRules.maxRequestTimeoutSeconds} value={model.requestTimeoutSeconds} onChange={(event) => setModel({ ...model, requestTimeoutSeconds: Number(event.target.value) })} /><small>单次 turn 最多 {data.modelRules.maxRequestTimeoutSeconds} 秒</small></label></div><ModelNotice notice={notice} /><div className="form-actions"><button className="ghost-button" disabled={savingConfig} onClick={() => onSaveConfig({ ...config, apiKey, clearApiKey })}>{savingConfig ? '保存连接中…' : '保存 Codex 连接'}</button><button className="ghost-button" disabled={testing || !config.configured} onClick={onTest}>{testing ? '正在执行实际 app-server 测试…' : '测试 app-server'}</button><button className="primary-button" disabled={saving} onClick={onSave}>{saving ? '保存中…' : '保存模型配置'}</button></div></div>
}
