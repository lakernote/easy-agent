...
export type SettingsSection = 'runtime' | 'tasks' | 'models' | 'skills' | 'tools' | 'usage' | 'security'
    ...
    <div className={`runtime-workbench settings-view-${settingsSection}`}>
      ...
      <div className="runtime-main">
        ...
        {settingsSection === 'tools' && <>
          {!codex && <div className="section-block"><div className="section-heading"><div><h2>EasyAgent 内置 Tools</h2><p>首轮只发送少量核心 Schema；文件、Shell 和网页工具需要时再加载。</p></div><span className="tag">{data.builtinTools.length} 个</span></div><div className="capability-note"><strong>工作区</strong><span><code>{data.runtime.workspace}</code></span><strong>私有 Runtime</strong><span><code>{data.runtime.runtime}</code></span></div><div className="tool-table">{data.builtinTools.map((tool) => <div key={tool.name}><code>{tool.name}</code><span>{tool.description}</span><em>{tool.category || tool.source}</em></div>)}</div></div>}
