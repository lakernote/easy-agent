  ...
  return <section className={`settings-page capabilities ${codex ? 'codex' : 'easyagent'} ${section === 'tools' ? 'extensions-page' : 'runtime-page'}`}>
    ...
    <div className={`runtime-workbench settings-view-${settingsSection}`}>
      {(section === 'runtime' || section === 'settings') && settingsSection === 'runtime' && <nav className="runtime-rail" aria-label="选择 Agent Runtime">
        <div className="runtime-rail-head"><div><p className="eyebrow">Runtime</p><strong>选择执行引擎</strong></div></div>
        ...
        <div className="runtime-rail-foot">新会话可临时切换，已有会话保持不变。</div>
      </nav>}
      <div className="runtime-main">
        ...
        <div className="runtime-summary"><div><p className="eyebrow">代表配置</p><strong>{model.profileName || '尚未保存配置'}</strong><span>{codex ? 'Codex Runtime' : 'EasyAgent Runtime'} · {model.model || (codex ? '跟随 ~/.codex/config.toml' : '未填写模型')}</span></div><div><p className="eyebrow">可用配置</p><strong>{selectedRuntimeProfiles.length} 套</strong></div><button className="ghost-button" type="button" onClick={openRuntimeModels}>管理 {codex ? 'Codex' : 'EasyAgent'} 配置 <span aria-hidden="true">→</span></button></div>
        </>}
        ...
        {settingsSection === 'tools' && <>
          {!codex && <div className="section-block"><div className="section-heading"><div><h2>EasyAgent 内置 Tools</h2><p>当前时间、Shell、只读文件检索和网页研究首轮可用；写入与低频能力按需加载。</p></div><span className="tag">{data.builtinTools.length} 个</span></div><div className="capability-note"><strong>工作区</strong><span><code>{data.runtime.workspace}</code></span><strong>私有 Runtime</strong><span><code>{data.runtime.runtime}</code></span></div><div className="capability-note"><strong>Research Provider</strong><span><code>TAVILY_API_KEY</code>、<code>BRAVE_SEARCH_API_KEY</code>、<code>EASYAGENT_SEARXNG_URL</code> 与 <code>GITHUB_TOKEN</code>/<code>GH_TOKEN</code> 从 EasyAgent 服务进程环境读取，修改后需重启；私有仓库、Issue 和 PR 请使用下方 GitHub MCP。</span></div><div className="tool-table">{data.builtinTools.map((tool) => <div key={tool.name}><code>{tool.name}</code><span>{tool.description}</span><em>{tool.category || tool.source}</em></div>)}</div></div>}
